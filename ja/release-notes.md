...
* [コンソール][API] コンテナ設定項目数の制限を追加
    * アクセスポリシー（ACL）は、読み取り、書き込み、照会の権限ごとに最大 100 個まで設定できます。
    * IP アクセス制御（IP ACL）は、許可 IP リストとブロック IP リストにそれぞれ最大 100 個まで設定できます。
    * CORS 許可オリジン（Origin）と公開する応答ヘッダーは、それぞれ最大 100 個まで設定できます。
    * ライフサイクル条件ルールは最大 30 個まで設定できます。
* [API] Amazon S3 API 互換性の改善
...
* [API] Amazon S3互換APIを使用して同じ名前のマルチパートオブジェクトを更新した時に、以前のパートオブジェクトが削除されない問題を修正しました。
...
* [コンソール] プレフィックス検索
    * 入力したプレフィックスで始まるコンテナ、フォルダ、オブジェクトを検索できるように検索機能を改善しました。
...
* [コンソール] リソースが残っている状態でサービスの使用を終了する際の案内文言を修正
