...
基本アクセスポリシーを設定し、テナントまたは個別ユーザー単位でロールベースのアクセスポリシーを管理します。アクセスポリシーの詳細については[アクセスポリシー設定ガイド](acl-guide/)を参考にしてください。
...
IPベースのアクセスポリシーを管理します。アクセスポリシーの詳細については[アクセスポリシー設定ガイド](acl-guide/)を参考にしてください。
...
ライフサイクル設定の詳細な説明は、[ライフサイクルルールの適用方法](container-policy-guide/#lifecycle-apply)を参考にしてください。
...
* **基本動作**
    * 元のコンテナでオブジェクトを変更(アップロード、メタデータの更新、削除)すると、複製対象コンテナに同じように反映されます。
    * 複製対象コンテナで変更されたオブジェクトは、元のコンテナに反映されません。
    * 複製はオブジェクトの最終変更時刻を基準に動作します。複製対象コンテナのオブジェクトが元のオブジェクトより後に変更されていた場合、複製されません。
* **設定時の注意事項**
    * 複製対象コンテナとしては空のコンテナを使用することをお勧めします。対象コンテナに元のコンテナのオブジェクトと同じ名前のオブジェクトが既にある場合、複製がスムーズに行われない可能性があります。
    * 複製対象コンテナに同じ名前のオブジェクトが削除された履歴がある場合、複製されたオブジェクトの最終更新時刻が複製設定時刻に変更される可能性があります。
    * 元のコンテナと複製対象コンテナの名前を同一に設定することをお勧めします。コンテナの名前が異なると、複製された大容量オブジェクトへのアクセスに失敗する可能性があります。
    * マルチパートアップロードした大容量オブジェクトのセグメントオブジェクトが別のコンテナに保存されている場合、セグメントが保存されたコンテナも同じ方法で複製設定を行わなければ、複製された大容量オブジェクトにアクセスできません。
* **設定変更時の注意事項**
    * 複製設定を「使用しない」に変更すると複製は中断されますが、すでに複製されたオブジェクトは維持されます。
    * 複製の方向を切り替えると、複製対象コンテナのオブジェクトが元のコンテナに復元されます。元のコンテナから削除されたオブジェクトも復元対象に含まれ、復元されたオブジェクトの最終更新時刻は複製設定時刻に変更されます。
    * 複製対象コンテナを削除すると、同じ名前のコンテナを再作成しても複製は再開されません。複製を再開するには、複製設定をやり直す必要があります。
* **制限及び例外事項**
    * 複製対象コンテナを別のリージョンに複製したり、別のコンテナの複製対象コンテナとして重複設定したりすることはできません。
    * ライフサイクルが期限切れになったもののまだ削除されていないオブジェクトは、対象コンテナに複製される際にライフサイクルの設定が削除されます。その後、元のコンテナから削除されると、削除履歴が対象コンテナに伝播されて削除されます。
...
!!! tip "メモ"
    Webコンソールでは5GBを超えるファイルはアップロードできません。アップロードするオブジェクトの容量が5GBを超える場合は、`split`などのコマンドラインツールを使用して分割するか、ユーザーアプリケーションで5GB以下のサイズに分割してアップロードするようにプログラミングする必要があります。詳細な使用方法はAPIガイドの[マルチパートアップロード](api-guide/#multipart-upload)を参考にしてください。
...
ロールベースのアクセスポリシーに関係なく、設定した時間の間、指定したオブジェクトに自由にアクセスできる署名されたURLを作成します。詳細な使用方法は[署名されたURLガイド](presigned-url-guide/)を参考にしてください。
