...
<table class="it" style="padding-top: 15px; padding-bottom: 10px;">
  ...
  <tr>
    <td>ライフサイクル有効期限切れ時の動作</td>
    <td>ライフサイクルの有効期限が切れたオブジェクトの処理方法を選択します。</td>
  </tr>
  <tr>
    <td>対象コンテナ</td>
    <td>ライフサイクル有効期限切れ時の動作として<b>コンテナの移動</b>を選択した場合、オブジェクトを移動するコンテナを選択する必要があります。</td>
  </tr>
</table>
...
* **設定時の注意事項**
    * 複製対象コンテナとしては空のコンテナを使用することを推奨します。対象コンテナに元のコンテナのオブジェクトと同じ名前のオブジェクトが既にある場合、複製がスムーズに行われない可能性があります。
    * 複製対象コンテナに同じ名前のオブジェクトが削除された履歴がある場合、複製されたオブジェクトの最終更新時刻が複製設定時刻に変更される可能性があります。
    * 元のコンテナと複製対象コンテナの名前を同一に設定することを推奨します。コンテナの名前が異なると、複製された大容量オブジェクトへのアクセスに失敗する可能性があります。
