...
このドキュメントでは、NHN Cloudコンソールでオブジェクトストレージのコンテナとオブジェクトを管理する方法を説明します。
...
コンテナを作成します。オブジェクトストレージにオブジェクトをアップロードするには、必ず1つ以上のコンテナが必要です。{% if encrypt %} 暗号化を設定すると、アップロードしたオブジェクトを自動的に暗号化して保存します。{% endif %}
...
<table class="it" style="padding-top: 15px; padding-bottom: 10px;">
  ...
  <tr>
    {# ECの存在によって行が1つ増える場合があります #}
    <td rowspan="$[ '5' if ec else '4' ]$">コンテナの作成</td>
    <td>名前</td>
    <td>コンテナ名は最短3文字から最長63文字まで使用でき、英小文字、数字、`-`、`.`、`+`のみ入力できます。<br>コンテナ名の先頭と末尾は英字または数字にする必要があります。<br>IPアドレス形式の名前は使用できません。</td>
  </tr>
  <tr>
    <td rowspan="2">コンテナアクセスポリシー</td>
    <td><b>PRIVATE</b>: 許可されたユーザーのみがコンテナ内のオブジェクトにアクセスできます。</td>
  </tr>
  <tr>
    <td><b>PUBLIC</b>: 公開URLを通じて誰でもコンテナ内のオブジェクトにアクセスできます。</td>
  </tr>
  {%- if ec %}
  <tr>
    <td rowspan="2">ストレージクラス</td>
    <td><b>Standard</b>: デフォルトのクラスです。</td>
  </tr>
  <tr>
    <td><b>Economy</b>: アクセス頻度の低いデータを長期保管するのに適したクラスです。</td>
  </tr>
  {% else %}
  <tr>
    <td>ストレージクラス</td>
  ...
  <tr>
    <td>ロック期間</td>
    <td>オブジェクトロックの期間を日単位で入力します。</td>
  </tr>
  {%- if encrypt %}
  <tr>
    <td rowspan="2">暗号化設定</td>
    <td>暗号化</td>
    <td>オブジェクトの暗号化を使用するかどうかを選択します。</td>
  </tr>
  <tr>
    <td>対称鍵ID</td>
    <td>Secure Key Managerで管理する対称鍵IDを入力します。</td>
  </tr>
  {%- endif %}
</table>
...
Economy クラスのコンテナにアップロードされたオブジェクトには、最低保管期間30日が適用されます。30日以前に削除されたオブジェクトについても、残余保管期間に対する料金が課金されます。

Economy クラスのコンテナには、APIリクエスト 1,000 件ごとに料金が適用されます（HEAD/DELETE リクエストを除く）。
...
デフォルトのアクセスポリシーを設定し、テナントまたは個別ユーザー単位でロールベースのアクセスポリシーを管理します。アクセスポリシー設定の詳細については、[アクセスポリシー設定ガイド](acl-guide/)を参照してください。
...
<table class="it" style="padding-top: 15px; padding-bottom: 10px;">
  ...
  <tr>
    <td>テナントID</td>
    <td>アクセスを許可するテナントIDまたは <code>*</code>を入力します。テナントIDはコンソールの**API エンドポイント設定**ダイアログボックスで確認できます。</td>
  </tr>
  <tr>
    <td>APIユーザーID</td>
    <td>アクセスを許可するAPIユーザーIDまたは<code>*</code>を入力します。 APIユーザーIDはコンソールの**API エンドポイント設定**ダイアログボックスで確認できます。</td>
  </tr>
  <tr>
    <td>権限</td>
...
IPベースのアクセスポリシーを管理します。アクセスポリシー設定の詳細については、[アクセスポリシー設定ガイド](acl-guide/)を参照してください。
...
<table class="it" style="padding-top: 15px; padding-bottom: 10px;">
  ...
  <tr>
    <td>権限</td>
    <td>許可しないアクセス権限(<code>Read</code>、<code>Write</code>)を選択します。</td>
  </tr>
  <tr>
...
<table class="it" style="padding-top: 15px; padding-bottom: 10px;">
  ...
  <tr>
    <td>エラー文書</td>
    <td>静的Webサイトのエラー文書オブジェクトのサフィックス(suffix)を入力します。エラー文書サフィックスにはフォルダパスを含められません。<br>最大256バイト、英字、数字、一部特殊文字(<code>-</code>、<code>_</code>、<code>.</code>)のみ入力できます。</td>
  </tr>
</table>
...
ホワイトリストに拡張子 `exe`, `jpg` を設定すると、 `.exe`, `.jpg` の拡張子を持つオブジェクトのみアップロードできます。ファイル名 `example` を追加設定すると、`exe_example.exe`, `image_example.jpg`など、設定したファイル名と拡張子の両方を持つオブジェクトのみをアップロードできます。

ブラックリストの場合、拡張子 `exe`, `jpg` を設定すると、すべての `.exe`, `.jpg` の拡張子を持つオブジェクトはアップロードできません。 ファイル名 `example` を追加設定すると、 `test.exe`, `image.jpg` のような制限のあるファイルと `text_example.txt` のような制限されたキーワードを持つファイルもアップロードできません。
...
{% if release_2026_05 %}ライフサイクル設定方法の詳細については、[ライフサイクルルールの適用方法](container-policy-guide/#lifecycle-apply)を参照してください。{% endif %}
...
{%- endif %}
    ...
    <td>オブジェクトのライフサイクルを日単位で入力します。{% if not release_2026_05 %} 空白のままにすると、ライフサイクル設定が解除されます。{% endif %}</td>
  </tr>
  <tr>
    <td>ライフサイクル満了時の動作</td>
  ...
  <tr>
    <td>対象コンテナ</td>
    <td>ライフサイクル満了時の動作として<b>コンテナ移動</b>を選択した場合、オブジェクトの移動先コンテナを選択する必要があります。</td>
  </tr>
{%- if release_2026_05 %}
  <tr>
...
!!! tip "メモ"
    変更したライフサイクルルールは、設定後にアップロードしたオブジェクトにのみ適用されます。
{%- if release_2026_08 %}

設定できる条件ルールは最大30個です。この制限は[コンテナポリシー](container-policy-guide/#lifecycle)で設定する場合も同様に適用されます。
{%- endif %}
{%- if ec %}
    Standard クラスのコンテナに保存されたオブジェクトをライフサイクルに従って Economy クラスのコンテナに移動し、長期保管にかかるコストを削減できます。
...
**[ルールの一括適用]** ボタンをクリックすると、コンテナ内の全てのオブジェクトのライフサイクルをルールに従って一括で再設定できます。
ルールは優先順位に従って適用され、一括適用時点を基準にライフサイクルが再計算されます。
...
<table class="it" style="padding-top: 15px; padding-bottom: 10px;">
  ...
  <tr>
    <td>アーカイブオブジェクト<br>ライフサイクル</td>
    <td>オブジェクトの以前のバージョンのライフサイクルを日単位で入力します。空欄のままにするとライフサイクル設定が解除されます。</td>
  </tr>
</table>

!!! danger "注意"
    アーカイブコンテナを元のコンテナより先に削除すると、元のコンテナのオブジェクトを更新または削除する際にエラーが発生します。すでに削除した場合は、アーカイブコンテナを新たに作成するか、元のコンテナのバージョン管理ポリシーを解除することで解決できます。
{%- if encrypt %}
    暗号化コンテナをアーカイブコンテナとして指定した後、暗号化コンテナの対称鍵を Secure Key Manager サービスから削除すると、元のコンテナへのオブジェクトのアップロードおよび削除が失敗します。
...
レプリケーション設定を使用して、コンテナのオブジェクトを別のリージョンのコンテナに複製できます。レプリケーション設定は、ディザスタリカバリ (disaster recovery) のための機能で、元のリージョンのオブジェクトを対象リージョンに同一の状態で複製および管理します。複製はバックグラウンドで一定の周期ごとに実行されます。
...
<table class="it" style="padding-top: 15px; padding-bottom: 10px;">
  ...
  <tr>
    <td>複製コンテナ</td>
    <td>複製対象コンテナを入力するか、検索ボタンをクリックして選択します。</td>
  </tr>
</table>
...
{% if encrypt %}
!!! danger "注意"
    暗号化コンテナを複製対象コンテナとして指定した後、暗号化コンテナの対称鍵を Secure Key Manager サービスから削除すると、暗号化コンテナの複製に失敗します。
...
* [オブジェクトのコピー](#copy-or-move-object)
* [オブジェクトの移動](#copy-or-move-object)
{%- if release_2026_05 %}
* [ライフサイクルルールの一括適用](#set-object-lifecycle-batch)
{%- endif %}

!!! tip "ヒント"
    作業履歴は、作業終了日から90日間保管されます。

<a id="task-record-list"></a>
##### 作業一覧
保管中の作業一覧を照会できます。**[履歴のエクスポート]** ボタンをクリックして、終了した作業一覧をファイルとしてダウンロードできます。

<a id="task-record-detail"></a>
##### 作業詳細
作業ごとの詳細情報を照会できます。照会できる情報は次のとおりです。
...
<table class="it" style="padding-top: 15px; padding-bottom: 10px;">
  ...
  <tr>
    <td rowspan="10">基本情報</td>
    <td>作業ID</td>
    <td>作業の一意のIDを示します。</td>
  </tr>
  <tr>
    <td>コンテナ</td>
    <td>作業が実行されたコンテナ名を示します。</td>
  </tr>
  <tr>
    <td>実行パス</td>
    <td>コンテナで作業を要求したパスを示します。<br>コンテナを空にするように、すべてのオブジェクトを対象に実行されるか、コンテナのルートパスで実行された作業の場合は、空の値として表示されます。</td>
  </tr>
  <tr>
    <td>種類</td>
    <td>作業の種類を示します。</td>
  ...
  <tr>
    <td rowspan="6">状態</td>
    <td><b>待機中</b>: 作業作成直後、実行待機中の状態です。</td>
  </tr>
  <tr>
    <td><b>進行中</b>: 作業が実行中の状態です。</td>
  </tr>
  <tr>
    <td><b>完了</b>: 作業のすべてのオブジェクト処理が成功した後、終了した状態です。</td>
  </tr>
  <tr>
    <td><b>失敗</b>: 作業の一部のオブジェクト処理が失敗したまま終了した状態です。</td>
  </tr>
  <tr>
    <td><b>キャンセル中</b>: ユーザーのキャンセル要求を受けた後、キャンセル待機中の状態です。</td>
  </tr>
  <tr>
    <td><b>キャンセル</b>: キャンセルが完了して終了した状態です。</td>
  </tr>
  <tr>
    <td rowspan="6">進行 / 結果</td>
    <td>ユーザー</td>
    <td>作業を要求したユーザーのIDを示します。</td>
  </tr>
  <tr>
    <td>作業要求時刻</td>
    <td>ユーザーが作業を要求した時刻を示します。</td>
  </tr>
  <tr>
    <td>作業開始時刻</td>
    <td>ユーザーが要求した作業の処理が開始された時刻を示します。</td>
  </tr>
  <tr>
    <td>作業終了時刻</td>
    <td>作業が終了した時刻を示します。</td>
  </tr>
  <tr>
    <td>所要時間合計</td>
    <td>作業が開始されてから終了するまでに要した時間を示します。</td>
  </tr>
  <tr>
    <td>進行カウンター</td>
    <td>処理を試みたオブジェクトの総数と、成功および失敗した数を示します。</td>
  </tr>
  <tr>
    <td>失敗結果</td>
    <td>失敗結果を表示</td>
    <td>失敗したオブジェクトの一覧と、オブジェクトごとの失敗理由および要求時刻などを確認できます。</td>
  </tr>
</table>

オブジェクトのコピー/移動作業には、次の情報が追加で表示されます。
...
<table class="it" style="padding-top: 15px; padding-bottom: 10px;">
  ...
  <tr>
    <td rowspan="2">コピー/移動先情報</td>
    <td>移動先コンテナ</td>
    <td>オブジェクトがコピー/移動されるコンテナ名を示します。</td>
  </tr>
  <tr>
    <td>移動先パス</td>
    <td>移動先コンテナでオブジェクトがコピー/移動されるパスを示します。</td>
  </tr>
</table>
...
### オブジェクトアップロード { #upload-object }
すべてのオブジェクトはコンテナ内にアップロードする必要があります。オブジェクト1つの最大容量は5GBに制限されます。

!!! tip "ヒント"
    コンソールでは、5 GB を超えるファイルはアップロードできません。アップロードするオブジェクトのサイズが 5 GB を超える場合は、`split` などのコマンドラインツールを使用して分割するか、ユーザーアプリケーションで 5 GB 以下のサイズに分割してアップロードするようにプログラムする必要があります。詳細については、APIガイドの[マルチパートアップロード](api-guide/#multipart-upload)を参照してください。
...
Python3のhttpモジュールを使用してWebサーバーを実行します。
...
Webブラウザで**http://localhost:8000**に接続した後、**Download**をクリックしてファイルが正常にダウンロードできるかを確認します。

</details>

<a id="copy-or-move-object"></a>
### オブジェクトコピー/移動 { #copy-or-move-object }
...
### 署名されたURLの作成 { #create-signed-url }
役割ベースのアクセスポリシーに関係なく、設定した時間の間、指定したオブジェクトに自由にアクセスできる署名付き URL を生成します。{% if release_2026_08 %} 詳しい使用方法については、[署名付き URL ガイド](presigned-url-guide/)を参照してください。{% endif %}
