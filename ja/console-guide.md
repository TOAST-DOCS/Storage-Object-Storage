...
複製設定を利用してコンテナのオブジェクトを他のリージョンのコンテナに複製するように設定できます。複製設定は、災害復旧(disaster recovery)のための機能で、対象リージョンのオブジェクトを元のリージョンと同じように複製して管理します。そのコンテナのすべてのオブジェクトが対象リージョンのコンテナに一定サイクルで複製されます。

複製ポリシーは次のとおりです。
* ソースコンテナでオブジェクトをアップロードまたは削除すると、すべて複製対象コンテナに同じように反映されます。
* 複製対象コンテナにアップデートした内容はソースコンテナに反映されません。
* 対象コンテナのオブジェクトと同じ名前のオブジェクトをソースコンテナにアップロードすると、対象コンテナのオブジェクトはソースコンテナでアップロードしたオブジェクトにアップデートされます。
* 複製設定を[使用しない]に変更すると、複製済みのオブジェクトは維持され、新しい複製は中断されます。
* マルチパートアップロードした大容量オブジェクトのセグメントオブジェクトが他のコンテナに保存されている場合、そのコンテナも複製設定を行ったときのみ大容量オブジェクトの複製が可能です。
* ソースコンテナと対象コンテナの名前は同じにすることを推奨します。同じでない場合、複製された大容量オブジェクトのアクセスに失敗する可能性があります。

> [注意]
> 複製対象コンテナ内の複製されたオブジェクトを削除した場合、再複製は行われません。

> [参考]
> 複製対象コンテナを他のリージョンに複製したり、他のコンテナの複製対象コンテナに重複設定できません。
#### 静的Webサイト設定
