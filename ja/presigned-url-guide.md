...
署名付き URL (presigned URL) は、シークレットキーであらかじめ署名した一時的なアクセスリンクです。**URL にはアクセス対象のオブジェクト、許可する HTTP メソッド (GET/PUT)、有効期限**が含まれており、これをシークレットキーで署名します。
...
Swift の Temporary URL は、オブジェクトの URL にクエリパラメータが追加された形式です。
...
| temp_url_prefix | N        | プレフィックス単位で署名する場合に必要です |
...
NHN Cloud オブジェクトストレージは S3互換API を提供しており、このときに生成される署名付き URL は次のような形式です。
...
| X-Amz-Credential | Y        | Access Key ID と署名が有効な scope (リージョン・サービス) を提供します。形式: `{access-key-id}/{date}/{region}/{service}/aws4_request` (サービスは `s3`、リージョンは `kr1` など)。URL 内の `/` は `%2F` でエンコードします |
| X-Amz-Date | Y        | リクエスト日時。ISO 8601 形式 `yyyyMMddTHHmmssZ` (UTC) で表現します。<br>例: `20260601T223241Z`                                                 |
| X-Amz-Expires | Y        | 署名付き URL の有効期間 (秒)。最小 `1`、最大 `604800` (7 日) |
...
署名付き URL を生成するには、まず署名対象オブジェクトの場所を決め、方式ごとに必要なキーと認証情報を準備します。署名対象はストレージエンドポイントとオブジェクトパスによって決まります。
...
TempURL は、ストレージアカウントまたはコンテナにあらかじめ登録しておいた秘密鍵 (Secret Key) で署名します。
...
!!! tip "ヒント"
    オブジェクトストレージは、ストレージアカウントごとに 2 個、コンテナごとに 2 個の秘密鍵の値を保存できます。

    リクエストを検証する際、オブジェクトストレージはすべてのキーの署名を確認します。各レベルでキーを 2 つ使用することで、既存の Temporary URL を無効化することなくキーをローテーションできます。

Swift CLI を使用して、次のように秘密鍵を設定できます。
...
## 署名付き URL の作成 { #create-presigned-url }

事前準備が完了したら、署名付き URL を作成する方法を説明します。
...
Temporary URL はプレフィックスベースでも作成できます。プレフィックスで始まるすべてのオブジェクトに有効な署名を生成できます。以下は、プレフィックスベースの Temporary URL 用 HMAC-SHA512 署名の例です。
...
Temporary URL を作成するには、このパスの前にオブジェクトストレージのホスト名を付加します。次のとおりです。
...
public class PresignPut {
    public static void main(String[] args) {
        ...
        PutObjectPresignRequest preq = PutObjectPresignRequest.builder()
            ...
            .putObjectRequest(put)                       // GET の場合は getObjectRequest
            .build();
...
署名付き URL には認証情報が含まれているため、認証トークンや署名ヘッダーなしで署名付き URL にリクエストを送信できます。どちらの方式（TempURL、SigV4）でも使用方法は同じです。
