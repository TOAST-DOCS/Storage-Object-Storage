...
기본 접근 정책을 설정하고, 테넌트 또는 개별 사용자 단위로 역할 기반 접근 정책을 관리합니다. 접근 정책에 대한 자세한 내용은 [접근 정책 설정 가이드](acl-guide-ngsc/)를 참고합니다.
...
IP 기반 접근 정책을 관리합니다. 접근 정책에 대한 자세한 내용은 [접근 정책 설정 가이드](acl-guide-ngsc/)를 참고합니다.
...
수명 주기 설정에 대한 자세한 설명은 [수명 주기 규칙 적용 방법](container-policy-guide-ngsc/#lifecycle-apply)을 참고합니다.
...
!!! tip "알아두기"
    웹 콘솔에서는 5GB를 초과하는 파일은 업로드할 수 없습니다. 업로드할 오브젝트의 용량이 5GB를 초과한다면 `split` 등의 명령줄 도구를 사용해 나누거나, 사용자 애플리케이션에서 5GB 이하의 크기로 나누어 업로드하도록 프로그래밍해야 합니다. 자세한 사용 방법은 API 가이드의 [멀티파트 업로드](api-guide-ngsc/#multipart-upload)를 참고합니다.
