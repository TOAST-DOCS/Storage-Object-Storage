...
```bash
https://kr4-api-object-storage.ninc.go.kr/v1/my_account/container/object
...
&X-Amz-Credential={your-access-key-id}/20260601/kr4/s3/aws4_request
...
| X-Amz-Credential | Y | Access Key ID와 서명이 유효한 scope(리전·서비스)를 제공. 형식: `{access-key-id}/{date}/{region}/{service}/aws4_request` (서비스는 `s3`, 리전은 `kr4` 등). URL에서 `/`는 `%2F`로 인코딩 |
...
!!! tip "알아두기"
    Swift CLI를 사용하려면 먼저 인증이 필요합니다. 자세한 내용은 [Swift CLI 환경설정](cli-guide-ninc/#configuration)을 참고합니다.
...
S3 호환 API를 사용하려면 먼저 AWS EC2 형태의 S3 API 자격 증명(Access Key ID + Secret Access Key)을 발급받아야 합니다. 자격 증명은 웹 콘솔 또는 API를 사용하여 발급받을 수 있습니다. 웹 콘솔을 사용한 자격 증명 발급은 [S3 API 자격 증명](console-guide-ninc/#s3-api-credentials) 항목을 참고합니다.
...
`aws` CLI 또는 SDK로 서명을 생성하려면, 발급받은 자격 증명을 로컬에 설정해야 합니다. 자세한 내용은 [Amazon S3 호환 API 가이드](s3-api-guide-ninc/#aws-command-line-interface-configuration) 설정 항목을 참고합니다.
...
```bash
https://kr4-api-object-storage.ninc.go.kr/v1/AUTH_6dbc368b94894416bec4cdfc65b5e067/my-container/cat.jpg?temp_url_sig=8244bff5037316dbe8aebcda9cd679c1b331e4790a1b2c3d4e5f60718293a4b5&temp_url_expires=1772755199
```
...
aws s3 presign s3://my-container/cat.jpg \
  --expires-in 3600 \
  --endpoint-url https://kr4-api-object-storage.ninc.go.kr
```
...
s3 = boto3.client(
    's3',
    endpoint_url='https://kr4-api-object-storage.ninc.go.kr',
    region_name='kr4',
...
public class PresignPut {
    public static void main(String[] args) {
        S3Presigner presigner = S3Presigner.builder()
            .region(Region.of("kr4"))
            .endpointOverride(URI.create("https://kr4-api-object-storage.ninc.go.kr"))
...
```bash
# Swift TempURL
curl -O "https://kr4-api-object-storage.ninc.go.kr/v1/AUTH_6dbc368b94894416bec4cdfc65b5e067/my-container/cat.jpg?temp_url_sig=8244bff5037316dbe8aebcda9cd679c1b331e4790a1b2c3d4e5f60718293a4b5&temp_url_expires=1772755199"
```

```bash
# S3 SigV4
curl -O "https://kr4-api-object-storage.ninc.go.kr/my-container/cat.jpg?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=253a3c7ca27f4731a9c757addfac29ca%2F20260601%2Fkr4%2Fs3%2Faws4_request&X-Amz-Date=20260601T201207Z&X-Amz-Expires=86400&X-Amz-SignedHeaders=host&X-Amz-Signature=8c1d9e2b5f4072a3b6c9d0e1f2a3b4c5d6e7f8091a2b3c4d5e6f70819a2b3c4d"
```
...
curl -X PUT -T ./cat.jpg \
  "https://kr4-api-object-storage.ninc.go.kr/v1/AUTH_6dbc368b94894416bec4cdfc65b5e067/my-container/cat.jpg?temp_url_sig=b1c4e9f2a8d7035641c2e9d8f4b1a7d063c5e8f9a2b1d4e70f3a8c1d9e2b5f40&temp_url_expires=1772755199"
```

```bash
# S3 SigV4
curl -X PUT -T ./cat.jpg \
  "https://kr4-api-object-storage.ninc.go.kr/my-container/cat.jpg?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=253a3c7ca27f4731a9c757addfac29ca%2F20260601%2Fkr4%2Fs3%2Faws4_request&X-Amz-Date=20260601T201207Z&X-Amz-Expires=86400&X-Amz-SignedHeaders=host&X-Amz-Signature=2b1d4e70f3a8c1d9e2b5f4076a3b8c1d9e2b5f40a1b2c3d4e5f60718293a4b50"
```
