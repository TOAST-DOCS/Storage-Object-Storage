...
기본 접근 정책을 설정하고, 테넌트 또는 개별 사용자 단위로 역할 기반 접근 정책을 관리합니다. 접근 정책에 대한 자세한 내용은 [접근 정책 설정 가이드](acl-guide-gov/)를 참고합니다.
...
IP 기반 접근 정책을 관리합니다. 접근 정책에 대한 자세한 내용은 [접근 정책 설정 가이드](acl-guide-gov/)를 참고합니다.
...
수명 주기 설정에 대한 자세한 설명은 [수명 주기 규칙 적용 방법](container-policy-guide-gov/#lifecycle-apply)을 참고합니다.
...
오브젝트 버전 관리를 설정하면 오브젝트의 이전 버전을 보관할 수 있습니다. 오브젝트를 업데이트 또는 삭제할 때 이전 버전이 지정된 아카이브 컨테이너에 보관됩니다. 이전 버전에 대한 수명 주기를 설정하면 수명 주기를 초과한 이전 버전은 자동으로 삭제됩니다.
...
!!! tip "알아두기"
    오브젝트 스토리지의 폴더는 파일 시스템이 제공하는 디렉터리와는 개념이 다릅니다. 편의를 위해 제공하는 의사(pseudo) 폴더입니다. 폴더를 생성하면 실제로는 `{폴더-이름}/`라는 빈 오브젝트가 만들어집니다. 폴더 안의 오브젝트는 `{폴더-이름}/{오브젝트-이름}` 형태의 이름을 가집니다. 오브젝트 복사 기능을 사용하여 새로운 폴더로 오브젝트를 복사하면 `{폴더-이름}/` 형태의 빈 오브젝트 없이 바로 `{폴더-이름}/{오브젝트-이름}` 형태의 오브젝트가 만들어집니다. 따라서 이 복사된 오브젝트를 삭제하면 폴더까지 삭제된 것처럼 보이게 됩니다. 미리 만든 폴더로 복사할 때는 폴더의 오브젝트를 삭제하더라도 폴더가 남아 있습니다.
...
!!! tip "알아두기"
    웹 콘솔에서는 5GB를 초과하는 파일은 업로드할 수 없습니다. 업로드할 오브젝트의 용량이 5GB를 초과한다면 `split` 등의 명령줄 도구를 사용해 나누거나, 사용자 애플리케이션에서 5GB 이하의 크기로 나누어 업로드하도록 프로그래밍해야 합니다. 자세한 사용 방법은 API 가이드의 [멀티파트 업로드](api-guide-gov/#multipart-upload)를 참고합니다.
