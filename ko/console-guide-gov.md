...
기본 접근 정책을 설정하고, 테넌트 또는 개별 사용자 단위로 역할 기반 접근 정책을 관리합니다. 접근 정책에 대한 자세한 내용은 [접근 정책 설정 가이드](acl-guide-gov/)를 참고하세요.
...
<table class="it" style="padding-top: 15px; padding-bottom: 10px;">
  ...
  <tr>
    <td>테넌트 ID</td>
    <td>접근을 허용할 테넌트 ID 또는 <code>*</code>를 입력합니다. 테넌트 ID는 콘솔의 API 엔드포인트 설정 대화 상자에서 확인할 수 있습니다.</td>
  </tr>
  <tr>
    <td>API 사용자 ID</td>
    <td>접근을 허용할 API 사용자 ID 또는 <code>*</code>를 입력합니다. API 사용자 ID는 콘솔의 API 엔드포인트 설정 대화 상자에서 확인할 수 있습니다.</td>
  </tr>
  <tr>
    <td>권한</td>
...
IP 기반 접근 정책을 관리합니다. 접근 정책에 대한 자세한 내용은 [접근 정책 설정 가이드](acl-guide-gov/)를 참고하세요.
...
화이트 리스트로 확장자 `exe`, `jpg`를 설정하면 `.exe`, `.jpg` 확장자를 가지는 오브젝트만 업로드할 수 있습니다. 파일명 `example`을 추가 설정하면 `exe_example.exe`, `image_example.jpg`와 같이 설정된 파일명과 확장자를 모두 가지는 오브젝트만 업로드할 수 있습니다.


블랙리스트의 경우 확장자 `exe`, `jpg`를 설정하면 모든 `.exe`, `.jpg` 확장자를 가지는 오브젝트는 업로드할 수 없습니다. 파일명 `example`을 추가 설정하면 `test.exe`, `image.jpg`와 같은 제한된 확장자의 파일과 `text_example.txt`와 같은 제한된 키워드를 가지는 파일 모두 업로드할 수 없습니다.
<br/>
...
수명 주기 설정에 대한 자세한 설명은 [수명 주기 규칙 적용 방법](container-policy-guide-gov/#lifecycle-apply)을 참고하세요.
<table class="it" style="padding-top: 15px; padding-bottom: 10px;">
  <tr>
    <th>유형</th>
    <th>항목</th>
...
<table class="it" style="padding-top: 15px; padding-bottom: 10px;">
  ...
  <tr>
    <td>대상 리전</td>
    <td>복제 대상 리전을 선택합니다. 프로젝트 구분이 동일 프로젝트라면 현재 이용 중인 리전은 제외됩니다.</td>
  </tr>
  <tr>
    <td>대상 컨테이너</td>
...
!!! tip "알아두기"
    웹 콘솔에서는 5GB를 초과하는 파일은 업로드할 수 없습니다. 업로드할 오브젝트의 용량이 5GB를 초과한다면 `split` 등의 명령줄 도구를 사용해 나누거나, 사용자 애플리케이션에서 5GB 이하의 크기로 나누어 업로드하도록 프로그래밍해야 합니다. 자세한 사용 방법은 API 가이드의 [멀티파트 업로드](api-guide-gov/#multipart-upload)를 참고하세요.
