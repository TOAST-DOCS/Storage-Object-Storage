...
## Storage > Object Storage > 콘솔 사용 가이드 { #storage-object-storage-console-guide }

이 문서는 NHN Cloud 콘솔에서 오브젝트 스토리지의 컨테이너와 오브젝트를 관리하는 방법을 설명합니다.
...
!!! danger "주의"
    복제 설정을 사용 중이면 대상 컨테이너의 오브젝트도 함께 삭제될 수 있습니다.

    컨테이너 비우기 작업을 진행 중인 컨테이너에 오브젝트를 업로드하면 삭제될 수 있습니다.

<a id="delete-container"></a>
### 컨테이너 삭제 { #delete-container }
선택한 컨테이너를 삭제합니다. 컨테이너를 삭제하기 전에 컨테이너가 비어 있는지 확인해야 합니다. 컨테이너 안에 오브젝트가 남아 있으면 삭제되지 않습니다.

<a id="manage-container"></a>
### 컨테이너 관리 { #manage-container }
...
기본 접근 정책을 설정하고, 테넌트 또는 개별 사용자 단위로 역할 기반 접근 정책을 관리합니다. 접근 정책 설정에 관한 자세한 내용은 [접근 정책 설정 가이드](acl-guide-gov/)를 참고합니다.
...
<table class="it" style="padding-top: 15px; padding-bottom: 10px;">
  ...
  <tr>
    <td>테넌트 ID</td>
    <td>접근을 허용할 테넌트 ID 또는 `*`를 입력합니다. 테넌트 ID는 콘솔의 **API 엔드포인트 설정** 대화 상자에서 확인할 수 있습니다.</td>
  </tr>
  <tr>
    <td>API 사용자 ID</td>
    <td>접근을 허용할 API 사용자 ID 또는 `*`를 입력합니다. API 사용자 ID는 콘솔의 **API 엔드포인트 설정** 대화 상자에서 확인할 수 있습니다.</td>
  </tr>
  <tr>
    <td>권한</td>
...
IP 기반 접근 정책을 관리합니다. 접근 정책 설정에 관한 자세한 내용은 [접근 정책 설정 가이드](acl-guide-gov/)를 참고합니다.
...
<table class="it" style="padding-top: 15px; padding-bottom: 10px;">
  ...
  <tr>
    ...
    <td>인덱스 문서</td>
    <td>정적 웹사이트의 인덱스 문서 오브젝트를 입력합니다. 오브젝트가 폴더 내에 있다면 폴더 경로를 포함해 입력해야 합니다.<br>최대 256 바이트, 영문자, 숫자, 일부 특수 문자(`-`, `_`, `.`, `/`)만 입력할 수 있습니다.</td>
  </tr>
  <tr>
    <td>오류 문서</td>
    <td>정적 웹사이트의 오류 문서 오브젝트의 접미사(suffix)를 입력합니다. 오류 문서 접미사에는 폴더 경로가 포함될 수 없습니다.<br>최대 256 바이트, 영문자, 숫자, 일부 특수 문자(`-`, `_`, `.`)만 입력할 수 있습니다.</td>
  </tr>
</table>
...
브라우저에서 Object Storage API를 직접 호출하려면 교차 출처 리소스 공유 설정이 필요합니다. 교차 출처 리소스 공유 항목의 변경 버튼을 클릭하면 허용할 출처 URL을 등록할 수 있습니다. URL에는 프로토콜(`https://` 또는 `http://`)이 포함되어야 합니다. `*`를 등록하면 모든 출처를 허용할 수 있습니다.
...
화이트리스트로 확장자 `exe`, `jpg`를 설정하면 `.exe`, `.jpg` 확장자를 가지는 오브젝트만 업로드할 수 있습니다. 파일명 `example`을 추가 설정하면 `exe_example.exe`, `image_example.jpg`와 같이 설정된 파일명과 확장자를 모두 가지는 오브젝트만 업로드할 수 있습니다.

블랙리스트로 확장자 `exe`, `jpg`를 설정하면 모든 `.exe`, `.jpg` 확장자를 가지는 오브젝트는 업로드할 수 없습니다. 파일명 `example`을 추가 설정하면 `test.exe`, `image.jpg`와 같은 제한된 확장자의 파일과 `text_example.txt`와 같은 제한된 키워드를 가지는 파일을 모두 업로드할 수 없습니다.
...
수명 주기 설정 방법에 관한 자세한 설명은 [수명 주기 규칙 적용 방법](container-policy-guide-gov/#lifecycle-apply)을 참고합니다.
...
<table class="it" style="padding-top: 15px; padding-bottom: 10px;">
  ...
  <tr>
    <td rowspan="3">기본 규칙</td>
    <td>오브젝트 수명 주기</td>
    <td>오브젝트의 수명 주기를 일 단위로 입력합니다.</td>
  </tr>
  <tr>
    <td>수명 주기 만료 동작</td>
    <td>수명 주기가 만료된 오브젝트 처리 방법을 선택합니다.</td>
  </tr>
  <tr>
    <td>대상 컨테이너</td>
  ...
  <tr>
    <td rowspan="5">조건 규칙</td>
    <td>규칙 이름</td>
    <td>수명 주기 규칙의 이름을 입력합니다.</td>
  </tr>
  <tr>
    <td>조건</td>
    <td>규칙이 적용될 조건을 지정합니다.</td>
  </tr>
  <tr>
    <td>오브젝트 수명 주기</td>
    <td>오브젝트의 수명 주기를 일 단위로 입력합니다.</td>
...
!!! tip "알아두기"
    ...
    설정할 수 있는 조건 규칙은 최대 30개입니다. 이 제한은 [컨테이너 정책](container-policy-guide-gov/#lifecycle)으로 설정할 때도 동일하게 적용됩니다.

<a id="set-object-lifecycle-batch"></a>
##### 규칙 일괄 적용

**규칙 일괄 적용** 버튼을 클릭하면 컨테이너 내 모든 오브젝트의 수명 주기를 규칙에 따라 일괄 재설정할 수 있습니다.
규칙은 우선순위대로 적용되며, 일괄 적용 시점을 기준으로 수명 주기가 다시 계산됩니다.

<a id="set-object-versioning"></a>
#### 오브젝트 버전

오브젝트 버전 관리를 설정하면 오브젝트의 이전 버전을 보관할 수 있습니다. 오브젝트를 업데이트 또는 삭제할 때 이전 버전이 지정된 아카이브 컨테이너에 보관됩니다. 이전 버전의 수명 주기를 설정하면 수명 주기를 초과한 이전 버전은 자동으로 삭제됩니다.
...
오브젝트 잠금 컨테이너의 오브젝트 잠금 주기를 확인하고 변경할 수 있습니다. 잠금 주기는 일 단위로 입력할 수 있으며, 해제할 수 없습니다.
...
!!! tip "알아두기"
    변경된 오브젝트 잠금 주기는 설정 변경 이후에 업로드한 오브젝트에 적용됩니다.
...
<table class="it" style="padding-top: 15px; padding-bottom: 10px;">
  ...
  <tr>
    <td rowspan="6">복제 설정</td>
    <td>복제</td>
    <td>복제 기능 사용 여부를 선택합니다.</td>
  </tr>
  <tr>
    <td rowspan="2">프로젝트 구분</td>
    <td><b>동일 프로젝트</b>: 복제 대상 컨테이너로 동일 프로젝트의 컨테이너를 선택합니다.</td>
  </tr>
  <tr>
    <td><b>다른 프로젝트</b>: 복제 대상 컨테이너로 다른 프로젝트의 컨테이너를 선택합니다.</td>
  </tr>
  <tr>
    <td>대상 프로젝트</td>
    <td>복제 대상 프로젝트를 입력합니다. 조회 버튼을 클릭하여 프로젝트의 권한을 확인할 수 있습니다.</td>
  </tr>
  <tr>
    <td>대상 리전</td>
    <td>복제 대상 리전을 선택합니다. 프로젝트 구분이 동일 프로젝트라면 현재 사용 중인 리전은 제외됩니다.</td>
  </tr>
  <tr>
    <td>대상 컨테이너</td>
    <td>복제 대상 컨테이너를 입력하거나 찾아보기 버튼을 클릭하여 선택합니다.</td>
  </tr>
</table>
...
* **기본 동작**
    * 원본 컨테이너에서 오브젝트를 변경(업로드, 메타데이터 업데이트, 삭제)하면 복제 대상 컨테이너에 동일하게 반영됩니다.
    * 복제 대상 컨테이너에서 변경된 오브젝트는 원본 컨테이너에 반영되지 않습니다.
    * 복제는 오브젝트의 최종 변경 시각을 기준으로 동작합니다. 복제 대상 컨테이너의 오브젝트가 원본보다 나중에 변경되었다면 복제되지 않습니다.
* **설정 주의 사항**
    * 복제 대상 컨테이너로는 빈 컨테이너 사용을 권장합니다. 대상 컨테이너에 원본 컨테이너의 오브젝트와 같은 이름의 오브젝트가 이미 있다면 복제가 원활하지 않을 수 있습니다.
    * 복제 대상 컨테이너에 동일한 이름의 오브젝트가 삭제된 이력이 있다면, 복제된 오브젝트의 최근 업데이트 시간이 복제 설정 시간으로 변경될 수 있습니다.
    * 원본 컨테이너와 복제 대상 컨테이너의 이름을 동일하게 설정하는 것을 권장합니다. 컨테이너의 이름이 다르면 복제된 대용량 오브젝트 접근에 실패할 수 있습니다.
    * 멀티파트 업로드한 대용량 오브젝트의 세그먼트 오브젝트가 다른 컨테이너에 저장되어 있다면, 세그먼트가 저장된 컨테이너도 같은 방식으로 복제 설정을 해야 복제된 대용량 오브젝트에 접근할 수 있습니다.
* **설정 변경 주의 사항**
    * 복제 설정을 사용 안 함으로 변경하면 복제는 중단되지만 이미 복제된 오브젝트는 유지됩니다.
    * 복제 방향을 전환하면, 복제 대상 컨테이너의 오브젝트가 원본 컨테이너로 복구됩니다. 원본 컨테이너에서 삭제된 오브젝트도 복구 대상에 포함되며, 복구된 오브젝트의 최근 업데이트 시간은 복제 설정 시간으로 변경됩니다.
...
중지된 컨테이너 복제를 다시 시작합니다. 복제는 중단 시점부터 이어서 수행됩니다.
...
컨테이너 복제를 중지합니다. 복제가 중지된 상태에서는 원본 컨테이너의 오브젝트를 삭제하거나 수정해도 복제되지 않습니다.

!!! danger "주의"
    복제 중지 기간 동안 삭제된 원본 컨테이너의 오브젝트는 대상 컨테이너에 반영되지 않을 수 있습니다.
...
* [오브젝트 복사](#copy-or-move-object)
* [오브젝트 이동](#copy-or-move-object)
* [수명 주기 규칙 일괄 적용](#set-object-lifecycle-batch)

!!! tip "알아두기"
    작업 이력은 작업 종료일로부터 90일간 보관됩니다.

<a id="task-record-list"></a>
##### 작업 목록
보관 중인 작업 목록을 조회할 수 있습니다. **이력 내보내기** 버튼을 클릭하여 종료된 작업 목록을 파일로 다운로드할 수 있습니다.

<a id="task-record-detail"></a>
##### 작업 상세
작업별 상세 정보를 조회할 수 있습니다. 조회할 수 있는 정보는 다음과 같습니다.
...
<table class="it" style="padding-top: 15px; padding-bottom: 10px;">
  ...
  <tr>
    <td rowspan="10">기본 정보</td>
    <td>작업 ID</td>
    <td>작업의 고유 ID를 나타냅니다.</td>
  </tr>
  <tr>
    <td>컨테이너</td>
    <td>작업이 실행된 컨테이너 이름을 나타냅니다.</td>
  </tr>
  <tr>
    <td>실행 경로</td>
    <td>컨테이너에서 작업을 요청한 경로를 나타냅니다.<br>컨테이너 비우기처럼 모든 오브젝트를 대상으로 수행되거나 컨테이너의 최상위 경로에서 실행된 작업이라면 빈 값으로 표시됩니다.</td>
  </tr>
  <tr>
    <td>유형</td>
    <td>작업의 유형을 나타냅니다.</td>
  ...
  <tr>
    <td rowspan="6">상태</td>
    <td><b>대기 중</b>: 작업 생성 직후 실행 대기 중인 상태입니다.</td>
  </tr>
  <tr>
    <td><b>진행 중</b>: 작업이 실행 중인 상태입니다.</td>
  </tr>
  <tr>
    <td><b>완료</b>: 작업의 모든 오브젝트 처리가 성공한 후 종료된 상태입니다.</td>
  </tr>
  <tr>
    <td><b>실패</b>: 작업의 일부 오브젝트 처리가 실패한 채 종료된 상태입니다.</td>
  </tr>
  <tr>
    <td><b>취소 중</b>: 사용자 취소 요청을 받은 후 취소를 대기 중인 상태입니다.</td>
  </tr>
  <tr>
    <td><b>취소</b>: 취소가 완료되어 종료된 상태입니다.</td>
  </tr>
  <tr>
    <td rowspan="6">진행 / 결과</td>
    <td>사용자</td>
    <td>작업을 요청한 사용자의 ID를 나타냅니다.</td>
  </tr>
  <tr>
    <td>작업 요청 시간</td>
    <td>사용자가 작업을 요청한 시간을 나타냅니다.</td>
  </tr>
  <tr>
    <td>작업 시작 시간</td>
    <td>사용자가 요청한 작업이 처리되기 시작한 시간을 나타냅니다.</td>
  </tr>
  <tr>
    <td>작업 종료 시간</td>
    <td>작업이 종료된 시간을 나타냅니다.</td>
  </tr>
  <tr>
    <td>총 소요 시간</td>
    <td>작업이 시작된 후 종료될 때까지 소요된 시간을 나타냅니다.</td>
  </tr>
  <tr>
    <td>진행 카운터</td>
    <td>처리를 시도한 오브젝트의 총 개수와 성공, 실패한 개수를 나타냅니다.</td>
  </tr>
  <tr>
    <td>실패 결과</td>
    <td>실패 결과 보기</td>
    <td>실패한 오브젝트 목록과 오브젝트별 실패 사유 및 요청 시간 등을 확인할 수 있습니다.</td>
  </tr>
</table>

오브젝트 복사/이동 작업에는 다음 정보가 추가로 표시됩니다.
...
<table class="it" style="padding-top: 15px; padding-bottom: 10px;">
  ...
  <tr>
    <td rowspan="2">복사/이동 대상 정보</td>
    <td>대상 컨테이너</td>
    <td>오브젝트가 복사/이동되는 컨테이너 이름을 나타냅니다.</td>
  </tr>
  <tr>
    <td>대상 경로</td>
    <td>대상 컨테이너에서 오브젝트가 복사/이동되는 경로를 나타냅니다.</td>
  </tr>
</table>
...
!!! tip "알아두기"
    오브젝트 스토리지의 폴더는 파일 시스템이 제공하는 디렉터리와는 개념이 다릅니다. 편의상 제공하는 의사(pseudo) 폴더입니다. 폴더를 생성하면 실제로는 `{폴더-이름}/`라는 빈 오브젝트가 만들어집니다. 폴더 안의 오브젝트는 `{폴더-이름}/{오브젝트-이름}` 형태의 이름을 가집니다. 오브젝트 복사 기능을 사용하여 새로운 폴더로 오브젝트를 복사하면 `{폴더-이름}/` 형태의 빈 오브젝트 없이 바로 `{폴더-이름}/{오브젝트-이름}` 형태의 오브젝트가 만들어집니다. 따라서 이 복사된 오브젝트를 삭제하면 폴더까지 삭제된 것처럼 보이게 됩니다. 미리 만든 폴더로 복사할 때는 폴더의 오브젝트를 삭제하더라도 폴더가 남아 있습니다.
...
!!! tip "알아두기"
    콘솔에서는 5GB를 초과하는 파일은 업로드할 수 없습니다. 업로드할 오브젝트의 용량이 5GB를 초과한다면 `split` 등의 명령줄 도구를 사용해 나누거나, 사용자 애플리케이션에서 5GB 이하의 크기로 나누어 업로드하도록 프로그래밍해야 합니다. 자세한 사용 방법은 API 가이드의 [멀티파트 업로드](api-guide-gov/#multipart-upload)를 참고합니다.
...
Python3의 http 모듈을 사용해 웹 서버를 실행합니다.
...
웹 브라우저에서 **http://localhost:8000**에 접속한 다음 **Download**를 클릭해 파일이 정상적으로 다운로드되는지 확인합니다.

</details>

<a id="copy-or-move-object"></a>
### 오브젝트 복사/이동 { #copy-or-move-object }
...
역할 기반 접근 정책에 관계없이 설정한 시간 동안 지정한 오브젝트에 자유롭게 접근할 수 있는 서명된 URL을 생성합니다. 자세한 사용 방법은 [서명된 URL 가이드](presigned-url-guide-gov/)를 참고합니다.
...
### 오브젝트 관리 { #manage-object }
선택한 오브젝트의 기본 정보를 확인하고 속성을 관리합니다.

!!! tip "알아두기"
    오브젝트 만료 날짜와 잠금 만료 날짜를 모두 설정한다면 오브젝트 만료 날짜는 항상 잠금 만료 날짜 이후로 설정해야 합니다.
...
검색 창에 접두사를 입력하고 **검색** 버튼을 클릭하면 입력한 접두사로 시작하는 컨테이너, 폴더, 오브젝트를 검색할 수 있습니다. 컨테이너 목록에서는 컨테이너를, 오브젝트 목록에서는 폴더와 오브젝트를 검색합니다.

<a id="s3-api-credentials"></a>
## S3 API 자격 증명 { #s3-api-credentials }
Amazon S3 호환 API를 사용하기 위한 자격 증명을 발급받을 수 있습니다. S3 API 자격 증명은 유효 기간이 없으며, 사용자별로 프로젝트당 최대 3개까지 발급받을 수 있습니다.

!!! danger "주의"
    S3 API 자격 증명 키가 유출되면 누구나 유출된 키를 사용하여 오브젝트에 접근할 수 있습니다. 키가 유출되었다면 유출된 자격 증명을 삭제하고 새로 발급받아 사용하세요.
