...
콘솔에서는 [컨테이너 생성](console-guide-ngcc/#create-container) 대화 상자 또는 [컨테이너 관리](console-guide-ngcc/#manage-container) 창의 컨테이너 접근 정책 설정 대화 상자에서 컨테이너 접근 정책을 선택할 수 있습니다. 선택할 수 있는 정책은 `PRIVATE`과 `PUBLIC` 두 가지로 제한됩니다.
...
!!! tip "알아두기"
    `X-Container-Read`, `X-Container-Write`, `X-Container-View`에 설정할 수 있는 접근 정책 요소는 각 속성별로 최대 100개입니다. 이 제한은 [컨테이너 정책](container-policy-guide-ngcc/#acl)으로 설정할 때도 동일하게 적용됩니다.
...
```
$ curl -O -X GET \
  http://ngcc-kr4-swift.kr.cloud.toastoven.net/v1/AUTH_*****/container/object
...
!!! tip "알아두기"
    `X-Container-Ip-Acl-Allowed-List`(화이트리스트)와 `X-Container-Ip-Acl-Denied-List`(블랙리스트)에 설정할 수 있는 정책 요소는 각각 최대 100개입니다. 이 제한은 [컨테이너 정책](container-policy-guide-ngcc/#ip-acl)으로 설정할 때도 동일하게 적용됩니다.
