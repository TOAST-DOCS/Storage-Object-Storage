...
##### 업로드 정책 설정 변경
컨테이너에 업로드할 오브젝트의 이름 기반의 정책을 설정합니다. `화이트리스트`와 `블랙리스트` 방식의 설정이 가능하며 두 방식을 동시에 설정할 수 없습니다. 파일의 확장자 또는 파일명에 포함되는 문자로 설정이 가능하며 폴더 오브젝트의 경우 업로드 정책에 제한되지 않습니다. 단, 경로가 포함된 오브젝트는 경로를 포함한 오브젝트명이 정책에 반영됩니다. 업로드 정책은 설정된 이후부터 신규 업로드되는 오브젝트에 적용됩니다.
<br/>
