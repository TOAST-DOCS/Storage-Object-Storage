...
복제 설정을 통해 컨테이너의 오브젝트를 다른 리전의 컨테이너에 복제하도록 설정할 수 있습니다. 복제 설정은 재해 복구(Disaster Recovery)를 위한 기능으로 대상 리전의 오브젝트를 원본 리전과 동일하게 복제하고 관리합니다. 해당 컨테이너의 모든 오브젝트가 대상 리전의 컨테이너에 복제되며 일정 주기로 복제가 이루어집니다.

복제 정책은 다음과 같습니다.
* 원본 컨테이너에서의 오브젝트 업로드, 업데이트, 삭제 행위는 모두 복제 대상 컨테이너에 동일하게 반영됩니다.
* 복제 대상 컨테이너에 업데이트한 내용은 원본 컨테이너에 반영되지 않습니다.
* 대상 컨테이너 내에 이미 존재하는 오브젝트와 같은 이름의 오브젝트를 원본 컨테이너에 업로드하면 대상 컨테이너의 오브젝트는 복제 정책에 따라 원본 컨테이너에서 업로드한 오브젝트로 업데이트됩니다.
...
컨테이너의 접근 정책을 **PUBLIC**으로 설정하고 인덱스 문서와 오류 문서를 입력하면 컨테이너에서 정적 웹 사이트 호스팅을 할 수 있습니다. 정적 웹 사이트 URL은 컨테이너 목록에서 [URL 복사] 버튼을 클릭해 얻을 수 있습니다.
