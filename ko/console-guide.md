...
<table class="it" style="padding-top: 15px; padding-bottom: 10px;">
  ...
  <tr>
    ...
    <td>복제</td>
    <td>복제 기능을 사용하도록 설정합니다.</td>
  </tr>
  <tr>
    <td>대상 리전</td>
    <td>선택한 리전의 컨테이너로 오브젝트를 복사합니다.</td>
  </tr>
  <tr>
    <td>복제 컨테이너</td>
...
컨테이너의 접근 정책을 설정할 수 있습니다. 접근 정책에는 **PRIVATE**과 **PUBLIC**이 있으며, **PRIVATE**로 설정된 경우 허가된 사용자만 내부 오브젝트에 접근할 수 있습니다.

#### 오브젝트 버전 관리

오브젝트 버전 관리 설정을 통해 오브젝트의 이전 버전을 보관할 수 있습니다. 이전 버전은 지정한 컨테이너에 보관되며, 설정을 통해 이전 버전에 대한 수명을 입력할 수 있습니다.

> [주의]
> 버전 관리 정책에 의한 아카이브 컨테이너를 원본 컨테이너보다 먼저 삭제하면, 원본 컨테이너의 오브젝트가 업데이트 또는 삭제될 때 문제가 발생합니다. 이미 삭제하였다면 아카이브 컨테이너를 새로 생성하거나 원본 컨테이너의 버전 관리 정책을 해제해 해결할 수 있습니다.

#### 오브젝트 복제

복제 설정을 통해 컨테이너의 오브젝트를 다른 리전의 컨테이너에 복제하도록 설정할 수 있습니다. 해당 컨테이너의 모든 오브젝트가 대상 리전의 컨테이너에 복제되며, 생성 이후 일정 주기로 복제가 이루어집니다. 원본 컨테이너에서 오브젝트를 삭제할 경우, 복제 대상 컨테이너 내의 복제된 오브젝트도 동일하게 삭제됩니다.

복제 대상 컨테이너에서의 변경사항은 원본 컨테이너에 반영되지 않습니다. 또한, 대상 컨테이너 내에 이미 존재하는 오브젝트와 같은 이름의 오브젝트를 원본 컨테이너에 업로드할 경우, 대상 컨테이너의 오브젝트는 복제 정책에 따라 원본 컨테이너에서 업로드한 오브젝트로 업데이트됩니다.
