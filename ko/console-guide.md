...
* 암호화 컨테이너의 오브젝트를 다른 컨테이너로 복사하거나 리전 간 컨테이너 복제로 다른 컨테이너에 복사하면, 대상 컨테이너의 암호화 설정에 따라 재암호화되거나 복호화되어 저장됩니다.
...
{% if release_2026_05 %}수명 주기 설정 방법에 관한 자세한 설명은 [수명 주기 규칙 적용 방법](container-policy-guide/#lifecycle-apply)을 참고합니다.{% endif %}
...
{%- endif %}
    <td>오브젝트 수명 주기</td>
    <td>오브젝트의 수명 주기를 일 단위로 입력합니다.{% if not release_2026_05 %} 공란으로 두면 수명 주기 설정이 해제됩니다.{% endif %}</td>
  </tr>
  <tr>
    <td>수명 주기 만료 동작</td>
...
역할 기반 접근 정책에 관계없이 설정한 시간 동안 지정한 오브젝트에 자유롭게 접근할 수 있는 서명된 URL을 생성합니다.{% if release_2026_08 %} 자세한 사용 방법은 [서명된 URL 가이드](presigned-url-guide/)를 참고합니다.{% endif %}
