...
접근 정책을 통해 접근 방식을 제한할 수 있습니다.
* **PRIVATE**: 허가된 사용자만 컨테이너 내부의 오브젝트에 접근할 수 있습니다.
* **PUBLIC**: 공개 URL을 통해 누구나 컨테이너 내부의 오브젝트에 접근할 수 있습니다.

##### 오브젝트 수명 주기

오브젝트 수명 주기를 설정한 컨테이너의 오브젝트는 설정된 수명 주기가 지난 후 자동으로 삭제됩니다.

#### 오브젝트 버전 관리 설정

오브젝트 버전 관리 설정을 통해 오브젝트의 이전 버전을 보관할 수 있습니다. 이전 버전은 지정한 아카이브 컨테이너에 보관되며, 설정을 통해 이전 버전에 대한 수명을 입력할 수 있습니다. 
...
복제 설정을 통해 컨테이너의 오브젝트를 다른 리전의 컨테이너에 복제하도록 설정할 수 있습니다. 복제 설정은 재해 복구(Disaster Recovery)를 위한 기능으로 대상 리전의 오브젝트를 소스 리전과 동일하게 복제하고 관리합니다. 해당 컨테이너의 모든 오브젝트가 대상 리전의 컨테이너에 복제되며 일정 주기로 복제가 이루어집니다.

복제 정책은 다음과 같습니다. 
* 원본 컨테이너에서의 삭제를 포함한 오브젝트 업데이트는 복제 대상 컨테이너 내의 복제된 오브젝트에도 동일하게 반영됩니다.
...
> [참고]
> 복제 대상 컨테이너를 대상으로는 다시 복제 설정을 할 수 없습니다.
