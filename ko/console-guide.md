...
* 대상 컨테이너의 오브젝트와 같은 이름의 오브젝트를 원본 컨테이너에 업로드하면 대상 컨테이너의 오브젝트는 원본 컨테이너에서 업로드한 오브젝트로 업데이트됩니다.
* 복제 설정을 [사용 안 함]으로 변경 시, 기존에 복제된 오브젝트는 유지되며 새로운 복제는 중단됩니다.
* 멀티 파트 업로드한 대용량 오브젝트의 세그먼트 오브젝트가 다른 컨테이너에 저장되어 있다면 해당 컨테이너도 복제 설정을 해야 대용량 오브젝트 복제가 가능합니다.
