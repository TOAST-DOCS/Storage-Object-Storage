...
##### 업로드 정책 설정 변경
컨테이너에 업로드할 오브젝트의 이름 기반의 정책을 설정합니다. 업로드 정책 설정을 활용하면 특정 확장자나 문구가 포함된 오브젝트만 업로드하거나 제한할 수 있습니다. 

업로드 정책 설정은 `화이트리스트`또는 `블랙리스트` 두 가지 방식을 지원하며, 두 방식을 동시에 설정할 수 없습니다. 파일의 확장자 또는 파일명에 포함되는 문자로 설정이 가능하며 폴더 오브젝트의 경우 업로드 정책에 제한되지 않습니다. 단, 경로가 포함된 오브젝트는 경로를 포함한 오브젝트명이 정책에 반영됩니다. 업로드 정책은 설정된 이후부터 신규 업로드되는 오브젝트에 적용됩니다. 

화이트리스트로 확장자 `exe`, `jpg`로 설정할 경우 `.exe`, `.jpg` 확장자를 가지는 오브젝트만 업로드 가능합니다. 파일명에 `example`로 설정을 추가한다면 `exe_example.exe`, `iamge_example.jpg`와 같이 설정된 파일명과 확장자 모두 만족한 오브젝트만이 업로드 가능합니다.

블랙리스트의 경우 확장자 `exe`, `jpg`로 설정한다면 모든 `.exe`, `.jpg` 이름을 가지는 오브젝트는 업로드 불가합니다. 파일명에 `exmaple`로 설정을 추가한다면 `test.exe`, `image.jpg`와 같은 확장자 제한의 파일은 물론 `text_example.txt`와 같은 파일 역시 파일명 제한으로 업로드 불가합니다.
<br/>
