...
수명 주기 설정에 대한 자세한 설명은 [수명주기 규칙 적용 방법](container-policy-guide/#lifecycle-apply)를 참고하세요.
<table class="it" style="padding-top: 15px; padding-bottom: 10px;">
  <tr>
    <th>유형</th>
    <th>항목</th>
...
`규칙 일괄 적용` 버튼을 클릭하면 컨테이너의 모든 오브젝트의 수명주기를 규칙에 따라 설정할 수 있습니다.
모든 수명 주기 규칙의 우선순위에 따라 규칙이 적용되며, 일괄 적용 시점을 기준으로 수명 주기가 다시 계산되어 적용됩니다.
...
> [참고]
> 웹 콘솔에서는 5GB를 초과하는 파일은 업로드할 수 없습니다. 업로드할 오브젝트의 용량이 5GB를 초과한다면 `split` 등의 명령줄 도구를 사용해 나누거나, 사용자 애플리케이션에서 5GB 이하의 크기로 나누어 업로드하도록 프로그래밍해야 합니다. 자세한 사용 방법은 API 가이드의 [멀티파트 업로드](api-guide/#multipart-upload)를 참조하세요.
