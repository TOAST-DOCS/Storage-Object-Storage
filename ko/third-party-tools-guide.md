...
브라우저 상단의 `새 연결` 버튼을 클릭하여 새로운 책갈피를 생성하고 추가 내용을 입력합니다.

드롭다운 메뉴에서 `Openstack Swift (Keystone 2.0)`를 선택합니다.
...
브라우저 좌측의 `Accounts` > `Add account`를 통해서 어카운트를 등록합니다. 등록에 필요한 정보는 아래와 같습니다.
...
입력값을 올바르게 입력한 뒤, 하단의 `Save`를 클릭해 새로운 어카운트를 등록합니다.
