...
콘솔에서는 [컨테이너 생성](console-guide-gov/#create-container) 대화 상자 또는 [컨테이너 관리](console-guide-gov/#manage-container) 창의 컨테이너 접근 정책 설정 대화 상자에서 컨테이너 접근 정책을 선택할 수 있습니다. 선택할 수 있는 정책은 `PRIVATE`과 `PUBLIC` 두 가지로 제한됩니다.
...
| 속성 | 설명 |
| --- | --- |
| X-Container-Read | 컨테이너 정보 조회와, 컨테이너 내 오브젝트 정보 조회 및 다운로드를 허용합니다. 일부 컨테이너의 정보는 조회가 제한됩니다. 컨테이너 및 오브젝트에 대한 GET, HEAD 요청이 해당됩니다. |
...
!!! tip "알아두기"
    `X-Container-Read`, `X-Container-Write`, `X-Container-View`에 설정할 수 있는 접근 정책 요소는 각 속성별로 최대 100개입니다. 이 제한은 [컨테이너 정책](container-policy-guide-gov/#acl)으로 설정할 때도 동일하게 적용됩니다.
...
!!! tip "알아두기"
    ...
    인증 토큰 발급 API를 이용하려면 API 가이드의 [인증 및 권한](api-guide-gov/#auth) 항목을 참고하세요.
...
```
$ curl -O -X GET \
  https://kr1-api-object-storage.gov-nhncloudservice.com/v1/AUTH_*****/container/object
