...
콘솔 또는 API를 사용해 화이트리스트, 블랙리스트를 지정하여 특정 IP에서의 컨테이너의 읽기/쓰기 접근 권한을 제한할 수 있습니다. 화이트 리스트와 블랙리스트는 동시에 사용할 수 없습니다. 화이트리스트와 블랙리스트가 같이 입력되어 있는 경우, 화이트 리스트만 사용됩니다. 접근 정책은 IPv4만 지원합니다.


> [주의]
> 잘못된 설정으로 인하여 쓰기 권한이 없는 컨테이너가 발생할 수 있습니다. 이 경우 더 이상 설정 변경이 불가능합니다. 이 경우, 고객센터로 문의가 필요합니다.
...
정책 요소는 접근 권한과, IP 또는 네트워크로 이루어져 있으며 콤마(`,`)로 구분해 여러 개의 값을 입력할 수 있습니다. 접근 권한은 아래의 표와 같습니다.
