...
콘솔 또는 API를 사용해 특정 IP에서의 컨테이너의 읽기/쓰기 접근 권한을 제한할 수 있습니다. 허용과 거부 정책 모두 동시에 사용은 불가하며, 두 정책이 모두 입력되어 있는 경우 허용 정책만이 유효합니다. 정책은 ipv4만 지원합니다.

## 정책 범위
IP 또는 네트워크 대역과 함께 정책 범위가 결정되어야 합니다. 이는 읽기 또는 쓰기, 모든 연산중 하나입니다.

## 허용 정책
허용된 IP 또는 네트워크 대역을 제외한 모든 요청을 거부합니다.

## 거부 정책
지정된 IP 또는 네트워크 대역의 요청을 거부합니다. 그 외의 모든 요청은 허용됩니다. 허용 정책과 함께 사용되는 경우 거부 정책은 무시됩니다.

## 콘솔

콘솔에서는 컨테이너 관리 창의 컨테이너 접근 정책 설정 대화 상자에서 IP 기반의 컨테이너 접근 정책을 선택할 수 있습니다. 

## API

API를 사용해 컨테이너의 `X-Container-Ip-Acl-Allowed-List`, `X-Container-Ip-Acl-Denied-List` 속성에 ACL 정책 요소를 입력하면 IP 기반의 ACL을 활성화할 수 있습니다.

<br><br>

정책 요소는 접근 방법과, IP 또는 네트워크로 이루어져 있으며 콤마(`,`)로 구분해 여러 개의 값을 입력할 수 있습니다. 접근 방법은 아래의 표와 같습니다.
...
192.168.0.1 IP는 읽기 요청만, 192.168.0.2 IP에서는 쓰기 요청만 가능하며 192.168.1.0/24 대역의 모든 IP는 모든 요청이 가능합니다. 그 외의 모든 IP는 요청이 거부됩니다.<br><br>
...
192.168.0.1 IP는 읽기 요청이, 192.168.0.2 IP에서는 쓰기 요청이 거부되며 192.168.1.0/24 대역의 모든 IP는 모든 요청이 불가능합니다. 그 외의 모든 IP는 요청이 허용됩니다.<br><br>
...
## 주의 사항
잘못된 설정으로 인하여 쓰기 권한이 없는 컨테이너가 발생할 수 있습니다. 이 경우 더 이상 설정 변경이 불가합니다. 이 경우, 고객센터로 문의가 필요합니다.
