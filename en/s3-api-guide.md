...
| PUT Object Lock Configuration | Set retention period for a locked bucket |
| GET Object Lock Configuration | Get Object Lock Retention Period |
...
If the user account that was issued the S3 API credentials loses access to the project or is deleted by leaving NHN Cloud, the credentials expire immediately and cannot be used.
...
!!! tip "Note"
    For more information about signature calculations for trailing headers, see [Signature calculations for trailing headers(chunked uploads)](https://docs.aws.amazon.com/AmazonS3/latest/API/sigv4-streaming-trailers.html).
...
### Create a Lock Bucket { #create-lock-bucket }
...
### Set Retention Period for Locked Bucket { #put-object-lock-configuration }
Sets the default retention period for a locked bucket.
...
| Rule.DefaultRetention.Years | Body | Integer | Conditional | Retention period (years). Positive integer. One of Days or Years is required. |
!!! tip "Note"
    If you omit `Rule`, the default retention period is set to 0 days.
    Even if you set the value in `Years`, it is always converted to and returned in `Days` when retrieved.
...
| Name | Type | Format | Description |
|---|---|---|---|
| ObjectLockEnabled | Body | String | Object lock enabled status |
...
!!! tip "Note"
    The retention period set in `Years` is also converted to `Days` and returned when retrieved.
...
A URL that embeds an **AWS Signature Version 4 (SigV4)** signature in query parameters, allowing access to an object for a limited time without an authentication token (Authorization header). Use `GET` for downloads and `PUT` for uploads.
...
| X-Amz-Credential | Query | String | Y | Access key and signature scope. Format: `{access}/{date}/{region}/s3/aws4_request` (`/` is encoded as `%2F`) |
...
!!! tip "Note"
    For more information, including the Swift TempURL method and direct signing examples by language, see the [Signed URL Guide](presigned-url-guide/).
...
!!! tip "Note"
    If the object size is 8 MB or larger, the AWS Command Line Interface splits the object into multiple parts and uploads them. Part objects are stored in a bucket named <code style="display: inline;">{bucket}+segments</code> with names in the format <code style="display: inline;">{object-name}/{upload-id}/{part-number}</code>. Once all parts are uploaded, an object that connects the part objects is created in the bucket where the upload was requested.

The <code style="display: inline;">{bucket}+segments</code> bucket where part objects are stored cannot be accessed with the S3-compatible API. It can be accessed through the Object Storage API or the console.

The ETag of a multipart object is the MD5 hash of the binary data obtained by converting the ETag value of each part object to binary data and concatenating them in order.

!!! danger "Caution"
    If you delete some or all part objects of a multipart-uploaded object, you cannot access the object.
...
!!! danger "Caution"
    If a bucket name contains a period (`.`), it may fall outside the valid scope of a wildcard SSL certificate when using the virtual-hosted-style URL, causing certificate validation to fail. In this case, use the path-style URL.
...
<code>create_bucket</code> Setting <code>ObjectLockEnabledForBucket=True</code> in the method creates a locked bucket. The default retention period is set to 0 days.
...
!!! tip "알아두기"
    The number of part objects is determined by the size of the uploaded object and the part size you set. The default part size is 8 MiB, and the minimum part size is 5 MiB. The maximum number of part objects is 10,000.
...
To retrieve the lock settings, use the <code>getObjectLockConfiguration</code> method.
    ...
    try
    {
        var request =
            new GetPreSignedUrlRequest
            {
                BucketName = bucketName,
                Key = keyName,
                Verb = HttpVerb.GET,                 // Use HttpVerb.PUT for upload
