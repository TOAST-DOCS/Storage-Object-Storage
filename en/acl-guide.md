...
This document describes how to configure role-based access policies and IP-based access policies for containers in NHN Cloud Object Storage.
...
!!! tip "Tip"
    The maximum number of access policy elements that can be set for `X-Container-Read`, `X-Container-Write`, and `X-Container-View` is 100 per property. This limit also applies when setting policies using [container policy](container-policy-guide/#acl).
...
!!! tip "Note"
    In the referrer settings, `*` can only be used as `.r:*`, which means public access for all. Values that combine `*` with other characters, `.r:-*` that blocks all access, and empty values cannot be used.
...
The HTTP referer access allow/block policy applies the block policy first, regardless of the order in which policies are entered. Therefore, access requests from HTTP referers designated as blocked are denied even if the `.r:*` policy element that allows all access is also entered.
...
!!! danger "Caution"
    If you do not have read permission, you can no longer operate on the container from the console.
...
You can use the API to activate IP-based ACL by entering IP-based access policy elements in the `X-Container-Ip-Acl-Allowed-List` and `X-Container-Ip-Acl-Denied-List` properties of a container. `X-Container-Ip-Acl-Allowed-List` refers to the allowlist, and `X-Container-Ip-Acl-Denied-List` refers to the blocklist.

!!! tip "Note"
    The maximum number of policy elements that can be set for `X-Container-Ip-Acl-Allowed-List` (whitelist) and `X-Container-Ip-Acl-Denied-List` (blacklist) is 100 each. This limit also applies when setting via [container policy](container-policy-guide/#ip-acl).
