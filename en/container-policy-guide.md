...
This document covers container policy and describes how to manage container-related settings in NHN Cloud Object Storage.
...
|---|---|---|
| `lifecycle` | Life cycle | Sets the life cycle rules for objects. |
| `acl` | Access control (ACLs) | Set the read, write, and view access permissions for the container. |
| `ip_acl` | IP access control (IP ACL) | Sets IP-based access control. |
| `cors` | Cross-Origin Resource Sharing (CORS) | Manages CORS settings, including allowed origins. |
| `lock` | Object lock | Sets the object lock (WORM) retention period. |
...
| policy | Query | String | Y | Query parameters for retrieving policies<br>If no value is specified, the entire policy document is returned. If a top-level key for a specific feature is specified, only the policy for that feature is returned. |
...
If no value is specified for the `policy` query parameter, the policies for all configured features are returned as a single document.
...
If a value is specified for the `policy` query parameter, only the policy for that feature is returned. Features that are not configured are not included in the response.

<details>
  <summary>Request and response example for querying a specific feature</summary>
...
When setting a policy, the following rules apply.

* Only `lifecycle`, `acl`, `ip_acl`, `cors`, and `lock` can be used as top-level keys. If you include a key or field that is not defined in the schema, the request is rejected.
* Top-level keys included in the policy document will overwrite the entire existing configuration with the submitted content, while settings for top-level keys that are not included will be retained as-is. If you send a top-level key with only some of its sub-items, the items that are not included will be removed. Therefore, you must always include all items that you want to retain.
...
!!! tip "Tip"
    ...
    If the policy document does not conform to the schema, an HTTP status code `400` is returned along with a message containing the error location and reason.
...
| `rules[*].action.type` | Enum | Y | Expiration behavior type | `"transfer"` (move) or `"delete"` (delete) |
| `rules[*].action.destination` | String | Conditional | Name of the target container to move the object to when it expires | Required when `type` is `"transfer"` |
<br>
...
Use the `acl` key in the container policy document to set container access permissions. Access control consists of three permissions: read, write, and get.
...
| View | `view` | Allows viewing the list of objects in the container. |
The `read`, `write`, and `view` in the policy document correspond to the `X-Container-Read`, `X-Container-Write`, and `X-Container-View` properties of a container, respectively. For more details on each permission, refer to the [Access Policy Configuration Guide](acl-guide/#role-based-access-api).
...
|---|---|---|---|---|
| `read` | Object | N | Read permission | Maximum 100 entries in total across `grantees`, `referrers` (`allow`, `deny`), `public`, and `listing` |
...
| `read.grantees` | Array | N | List of users to grant read permission | Corresponds to a role-based access policy element in the `<tenant>:<user>` format. |
...
| `view.grantees` | Array | N | List of users to grant view permission | The format of `grantees` is the same as `read.grantees`. |
!!! tip "Note"
    For elements corresponding to `public`, `listing`, and `referrers`, see [Common Access Policy Elements](acl-guide/#common-access-elements) in the Access Policy Configuration Guide. For elements corresponding to `grantees`, see [Role-Based Access Policy Elements](acl-guide/#role-based-access-elements).
...
Since `view` is not included, the view permission is revoked.
...
Use the `ip_acl` key in the container policy document to configure IP-based access control. Whitelists and blacklists cannot be used simultaneously; if both are set, only the whitelist is applied. For more information about how it works, see [IP-based access policies](acl-guide/#ip-based-access-policies) in the Access Policy Configuration Guide.
...
| Field | Format | Required | Description | Note |
|---|---|---|---|---|
| `whitelist` | Array | N | List of IP addresses to allow | Max. 100 items |
...
| `services` | Array | N | Access control by service | |
...
Use the `cors` key in the container policy document to configure Cross-Origin Resource Sharing (CORS). For more details on CORS settings and allowed origin formats, see [Cross-Origin Resource Sharing (CORS)](api-guide/#set-container-cors-policy) in the API guide.
...
| Field | Format | Required | Description | Note |
|---|---|---|---|---|
| `allow_origins` | Array | N | List of allowed origins | Max. 100 items<br>Cannot include a blank space in each item. |
| `max_age` | Integer | N | Preflight response cache time | In seconds, integer of 0 or greater |
...
Use the `lock` key in the container policy document to set the object lock (WORM, Write-Once-Read-Many) interval. For the concept and constraints of object lock, refer to [Change Object Lock Date](api-guide/#set-container-object-lock-cycle) in the API guide.
...
| Field | Format | Required | Description | Note |
|---|---|---|---|---|
| `days` | Integer | Y | Object lock cycle | In days, from 0 to 36,500 days (up to 100 years) |
<a id="lock-schema-application-rules"></a>
#### Application Rules

When configuring object locking, note the following:

* Object locking can only be enabled when creating a container. Requests to set `lock` on an existing container that does not have object locking configured will be rejected.
* Object locking cannot be disabled. Setting `days` to `0` only sets the default lock period to 0 days; object locking remains active.
